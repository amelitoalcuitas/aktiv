...
@section('title', 'Open Play Session Started — Aktiv')
...
@section('content')
    ...
    <div style="text-align:center; margin-bottom:28px;">
        ...
        <h1 style="margin:0 0 4px; font-size:1.25rem; color:#0f1728;">Your Session Has Started!</h1>
        <p style="color:#64748b; font-size:0.875rem; margin:0;">Head over to <strong>{{ $hub->name }}</strong> — the <strong>{{ $sport }}</strong> open play session is live.</p>
    </div>
    ...
    <div style="background:#f0f4f8; border-radius:8px; padding:16px; margin:24px 0; font-size:0.875rem;">
        <table style="width:100%; border-collapse:collapse;">
            ...
            @if($session->price_per_player > 0 && $participant->payment_method === 'pay_on_site')
            <tr>
                <td style="padding:4px 0; color:#64748b;">Payment</td>
                <td style="padding:4px 0; text-align:right; font-weight:500; color:#0f1728;">Pay ₱{{ number_format($session->price_per_player, 0) }} at the venue</td>
            </tr>
            @endif
        </table>
    </div>
